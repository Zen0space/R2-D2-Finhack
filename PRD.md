...
|---|---|
| **AI & Intelligent Systems** — *Effective and meaningful integration of AI to address the problem statements* | **Three layers across the project lifecycle**, not just two product-internal use-cases. **Layer 1 (pre-product):** ~2,400 lines of planning artifacts (PRD · architecture · pitch deck · narration · dev plan · brand · team docs) generated through multi-agent AI orchestration before product code began — every doc in this repo is evidence. **Layer 2 (process):** [`maji-core/`](./maji-core/) team coordinator ships with the repo — 6 slash commands (`/maji-onboard`, `/maji-whoami`, `/maji-phase`, `/maji-gate`, `/maji-pair`, `/maji-handoff`) enforce phase gates, schema-locked persistent memory, Akal coding discipline, Jimat communication register. **Layer 3 (in-product):** Penasihat catalogue suggester + NADI weekly summary with anomaly detection — multi-cloud routing (Alibaba Function Compute · Qwen primary · Anthropic Claude failover). Full 3-layer story: [`docs/ai-methodology.md`](./docs/ai-methodology.md). |
| **Technical Implementation** — *Scalability, robustness, security or prototype* | Append-only ledger · HMAC-verified webhooks · argon2 hashing · role-based NADI portal scoping · TypeScript strict end-to-end · explicit production scale path documented (read replica → ASG → Aurora multi-AZ) · rate limiting via `hono-rate-limiter` · security posture section in [ARCHITECTURE.md](./ARCHITECTURE.md). |
| **Multi-Cloud Service Usage** — *Effective and purposeful use of at least two or more cloud platforms* | **AWS + Alibaba Cloud — both sponsor-aligned (Gold + Platinum).** AWS hosts main backend (EC2 + Postgres + S3). Alibaba Cloud hosts AI workloads via Function Compute wrapping Qwen LLM. Catalogue images mirrored to Alibaba Cloud OSS. Failover to Anthropic Claude on Alibaba 5xx. **Multi-cloud is real, not theatrical** — actual code routing in [`backend/src/services/penasihat.ts`](./backend/src/services/penasihat.ts) and deployable Alibaba FC handler in [`alibaba-function-compute/penasihat-suggest/index.js`](./alibaba-function-compute/penasihat-suggest/index.js). |
| **Impact & Feasibility** — *Real-world use case relevance, sustainability and potential adoption* | Test bed: **NADI Felda Gedangsa, Hulu Selangor** — real Felda smallholder community with existing MCMC-run NADI centre. Real institutional package: TNG (Gold sponsor · 23M users) + NADI/MCMC (188 centres nationally · 84 in Selangor) + MyKasih (3.7M monthly SARA recipients · 10,000+ merchant network). 2.9M B40 households nationally. **No new welfare programme invented** — DuitLater composes existing rails. **No new credit instrument** — uses TNG's existing PayLater risk model. Sustainability: every partner is funded under permanent mandate. |
| **Presentation & Teamwork** — *Clarity in project demo and pitch, teamwork, documentation quality* | 4-min on-stage script in [docs/pitch-narration.md](./docs/pitch-narration.md) with recovery phrases per slide. 8-slide pitch deck in [docs/pitch-deck.md](./docs/pitch-deck.md). **Documentation quality is exceptional** — this 600+ line PRD, ARCHITECTURE with 8 mermaid diagrams, WORLD manifesto, BRAND visual identity guide, DEVELOPMENT-PLAN with phase-by-phase testable outcomes. **maji-core team coordinator** ([maji-core/](./maji-core/)) — 6 slash commands for daily R2-D2 operations · phase-gate enforcement · committed personal memory for cross-team visibility. /about-team page rendered with R2-D2 role cards at <http://localhost:3000/about-team>. |
...
- **F-AI4** **Multi-cloud routing** — `services/penasihat.ts` calls Alibaba Cloud Function Compute (Qwen-backed) when `ALIBABA_FUNCTION_COMPUTE_URL` is set; falls back to Anthropic Claude on 5xx or timeout. Both providers receive identical structured-output schema: `{ items: [{ itemId, itemName, priceCents, allocationPct, reasoningBm, reasoningEn }] }`
- **F-AI5** Response cached per pool for 30 min (avoid re-spending API quota on UI re-renders)
- **F-AI6** Provider used (alibaba-qwen | anthropic-claude) returned in response for observability

### 9.5 AI NADI Weekly Summary (2nd AI use-case · multi-cloud)
...
- **F-NS3** **Multi-cloud routing** — same pattern as Penasihat. Alibaba Function Compute (Qwen) primary, Anthropic Claude fallback. Output: `{ headline_bm, observations_bm: string[], anomalies_bm: string[], suggestion_bm }`
...
- **F-NS6** Audit-friendly — all summaries logged with provider used + generation timestamp
...
**Multi-cloud (AWS + Alibaba Cloud) · sponsor-aligned (Gold + Platinum) · 4-container AWS Docker spine + serverless Alibaba Function Compute for AI workloads.**

```
[ User Browser ]
       |
       v  HTTPS :443
[ AWS · Caddy + Frontend + Hono + Postgres ]
       |
       ├── /api/penasihat/suggest → [ Alibaba Cloud Function Compute · Qwen ]
       ├── /api/nadi/summary      → [ Alibaba Cloud Function Compute · Qwen ]
       ├── (Alibaba FC fails)     → [ Anthropic Claude · failover ]
       ├── (catalogue images)     → [ Alibaba Cloud OSS ]
       ├── (TNG transactions)     → [ TNG PayLater sandbox · simulated for demo ]
       └── (general assets)       → [ AWS S3 ]
```

**Multi-cloud rationale:**
- **AWS** (Gold sponsor) — main backend compute, Postgres ledger, S3 for general assets · familiar ops surface
- **Alibaba Cloud** (Platinum sponsor) — AI workloads (Penasihat suggester + NADI weekly summary) on Function Compute wrapping Qwen LLM · BM-native reasoning · data sovereignty narrative · cost-optimised for structured-output AI

**Failover preserved** — backend's AI service routers (`services/penasihat.ts`, `services/nadi-summary.ts`) call Alibaba FC primary, fall back to Anthropic Claude on 5xx or timeout.

Full diagrams + scale path + security posture: [ARCHITECTURE.md](./ARCHITECTURE.md). Tech stack inventory: [TECH-STACK.md](./TECH-STACK.md). Alibaba Cloud Function Compute deploy guide: [`alibaba-function-compute/README.md`](./alibaba-function-compute/README.md).
