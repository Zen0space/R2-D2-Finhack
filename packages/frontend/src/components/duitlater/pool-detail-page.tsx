...
function isAutoGeneratedEmail(email: string) {
  return /^(try-|demo-)/.test(email) && /@duitlater\.(local|demo)$/.test(email);
}

function formatShortShareLink(link: string) {
  try {
    const url = new URL(link);
    const code = url.pathname.split("/").filter(Boolean).pop() ?? "";
    return `${url.host}/join/${code}`;
  } catch {
    return link;
  }
}
      ...
      <div className="mx-auto grid w-full max-w-[1800px] gap-6">
          ...
          <Card>
            <CardHeader className="gap-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <Badge tone="forest">
                  {pool.members.length} / {pool.maxMembers} seats
                </Badge>
              </div>
              <CardTitle className="text-4xl">Who&rsquo;s in</CardTitle>
              <CardDescription className="text-base">
                The combined cap is the sum of each member&rsquo;s allowance below. Empty seats fill as neighbours join.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3">
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      {!isAutoGeneratedEmail(member.email) ? (
              ...
              {pool.state === "draft"
                ? Array.from({ length: Math.max(0, pool.maxMembers - pool.members.length) }).map(
                    (_, index) => (
                      <div
                        key={`open-seat-${index}`}
                        className="flex items-center justify-between gap-3 rounded-[1.5rem] border-2 border-dashed border-[color:var(--dl-sand)] bg-[color:rgba(248,244,236,0.4)] p-4"
                      >
                        <div className="flex items-center gap-3">
                          <span className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-dashed border-[color:var(--dl-slate)] text-[color:var(--dl-slate)] opacity-60">
                            +
                          </span>
                          <div>
                            <p className="zine-display text-sm uppercase tracking-[0.18em] text-[color:var(--dl-slate)]">
                              Open seat
                            </p>
                            <p className="text-xs text-[color:var(--dl-slate)] opacity-75">
                              Waiting for a neighbour to join
                            </p>
                          </div>
                        </div>
                        <span className="text-xs uppercase tracking-[0.18em] text-[color:var(--dl-slate)] opacity-60">
                          Seat {pool.members.length + index + 1}
                        </span>
                      </div>
                    ),
                  )
              ...
              {pool.state === "draft" && pool.members.length < 2 ? (
                <Button
                  variant="primary"
                  size="default"
                  ...
                  onClick={() =>
                    copyText(
                      shareLink,
                      "Link copied. Paste it in WhatsApp / Telegram to invite a neighbour.",
                    )
                  }
                >
                  <Share2 aria-hidden="true" size={16} />
                  Share invite to fill seats
                </Button>
              ) : null}
            </CardContent>
          </Card>
          ...
          <div className="grid gap-4">
            <Card>
              <CardHeader className="gap-3">
                ...
                <CardDescription className="text-base">
                  Send the code, link, or QR to neighbours so they can join the pool.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-4">
                <div className="grid gap-4 sm:grid-cols-[0.95fr_1.05fr]">
                  ...
                  <div className="grid gap-3 rounded-[1.5rem] border border-[color:var(--dl-sand)] bg-white/82 p-4">
                    ...
                    <p
                      className="rounded-[1rem] bg-[color:rgba(248,244,236,0.72)] px-3 py-2 font-mono text-xs text-[color:var(--dl-slate)]"
                      title={shareLink}
                    >
                      {formatShortShareLink(shareLink)}
                    </p>
                    <Button variant="outline" size="sm" onClick={() => copyText(shareLink, "Link copied.")}>
                      <Copy aria-hidden="true" size={14} />
                      Copy full link
                    </Button>
                  </div>
        ...
        {isQrModalOpen ? (
          <div
            role="dialog"
            aria-modal="true"
            aria-label="Pool invite QR"
            className="fixed inset-0 z-50 flex items-center justify-center bg-[color:rgba(26,26,26,0.72)] px-4"
            onClick={() => setIsQrModalOpen(false)}
          >
            <div
              className="relative w-full max-w-md rounded-[2rem] bg-[var(--dl-paper)] p-6 text-center"
              onClick={(event) => event.stopPropagation()}
              style={{ boxShadow: "8px 8px 0 var(--dl-zine-ink)" }}
            >
              <button
                type="button"
                onClick={() => setIsQrModalOpen(false)}
                aria-label="Close"
                className="absolute right-3 top-3 rounded-full border-2 border-[var(--dl-zine-ink)] bg-[var(--dl-paper)] p-1.5 hover:bg-[var(--dl-paper-warm)]"
              >
                <X size={16} />
              </button>
              ...
              <p className="zine-display mt-2 text-2xl text-[var(--dl-zine-ink)]">
                {pool.inviteCode}
              </p>
